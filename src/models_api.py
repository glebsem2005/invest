...
class ExcelSearchStrategy(ModelStrategy):
    ...
    def __init__(self) -> None:
        ...
        logger.info(f'[ExcelSearchStrategy] API KEY: {config.OPENAI_API_KEY[:15]}... (скрыт)')
        logger.info(f'[ExcelSearchStrategy] Vector Store ID: {self._file_manager._vector_store_id}')
    ...
    async def get_response(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            logger.debug(f'[ExcelSearchStrategy] Используем vector_store_ids: {self._file_manager._vector_store_id}')
