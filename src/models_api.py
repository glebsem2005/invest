...
class ChatGPTStrategy(ModelStrategy):
    ...
    async def get_response(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            logger.info(f'[{self.__class__.__name__}] Отправка запроса, модель: {self.model}, max_tokens: {self._max_tokens}')
            ...
            logger.info(f'[{self.__class__.__name__}] Получен ответ, длина: {len(content)} символов, использовано токенов: {token_usage}')
            return content
...
class ChatGPTFileStrategy(ModelStrategy):
    ...
    async def get_response(self, messages: List[Dict[str, str]]) -> str:
        """Отправляет запрос к ChatGPT и возвращает ответ."""
        try:
            logger.info(f'[{self.__class__.__name__}] Отправка запроса к файловой модели: {self.model}, max_tokens: {self._max_tokens}')
            logger.debug(f'[{self.__class__.__name__}] Сообщения: {messages}')
