...
class ExcelSearchStrategy(ModelStrategy):
    ...
    async def get_response(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            search_prompt = (
                'Найди в загруженном Excel файле наиболее подходящую информацию для запроса пользователя.\n'
                f"Запрос пользователя: {messages[-1]['content']}\n"
                'ОБЯЗАТЕЛЬНО верни заголовок таблицы и одну строку, которая наиболее подходит под запрос пользователя.\n'
