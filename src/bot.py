...
Ты помощник для интеграции вопросов и ответов в инвестиционный отчет.

Задача: возьми существующий отчет и интегрируй в него дополнительные вопросы и ответы, полученные в ходе диалога.

Правила:
1. Сохрани структуру оригинального отчета
2. Добавь информацию из Q&A в соответствующие разделы
3. Если информация из Q&A не подходит ни к одному разделу, создай новый раздел "Дополнительные вопросы"
4. Перефразируй Q&A так, чтобы они органично вписались в отчет
5. Добавь краткие выводы по каждому новому аспекту
6. Сохрани профессиональный стиль отчета
...
class InvestmentActionsHandler(BaseScenario):
    ...
    async def process(self, callback_query: types.CallbackQuery, state: FSMContext, **kwargs) -> None:
        ...
        if action == 'investment_regenerate':
            ...
            await callback_query.message.answer('🔄 Запускаю регенерацию анализа...')
...
class InvestmentQAHandler(BaseScenario):
    ...
    async def process(self, message: types.Message, state: FSMContext, **kwargs) -> None:
        ...
        try:
            ...
            context_prompt = f"""
            Ты инвестиционный аналитик. Отвечай на вопросы по компании {company_name}, 
            используя контекст из executive summary и предыдущих вопросов.
            ...
            model_api = ModelAPI(Models.chatgpt.value())
            messages = [
                {"role": "system", "content": "Ты эксперт по инвестиционному анализу. Отвечай точно и профессионально."},
    ...
    def _format_qa_history(self, qa_history):
        """Форматирует историю вопросов и ответов."""
        if not qa_history:
            return "Предыдущих вопросов нет."
        
        formatted = ""
        for i, qa in enumerate(qa_history, 1):
            formatted += f"Вопрос {i}: {qa['question']}\nОтвет {i}: {qa['answer']}\n\n"
        return formatted
